/*
───────────────────────────────────────────────
  InMemoryStore for WhatsApp Multi-Device Bots
───────────────────────────────────────────────

  • Purpose:
    - Provides a robust in-memory state management system for WhatsApp bots using Baileys.
    - Replaces the deprecated makeInMemoryStore from the official Baileys library.
    - Ensures compatibility with the latest Baileys event-driven architecture.

  • Features:
    - Stores contacts, chats, messages, presences, group metadata, call offers, sticker packs, authentication state, and sync history.
    - Handles all relevant events and updates state accordingly.
    - Includes error handling, input validation, and extensible event binding.
    - Designed for reliability, maintainability, and easy debugging.

  • Usage:
    - Import and instantiate via the provided factory function.
    - Bind to Baileys event emitters for automatic state synchronization.
    - Use provided methods to access, update, or clear state as needed.
